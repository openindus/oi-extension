...
void OICore::init()
{
    ...
    _etorCurrentMode[ETOR1] = NONE;
    _etorCurrentMode[ETOR2] = NONE;
    _etorCurrentMode[ETOR3] = NONE;
    _etorCurrentMode[ETOR4] = NONE;
...
int OICore::digitalRead(Etor_t etor) const
{
...
void OICore::attachFunctions(void)
{
    OIModule::attachFunctions();

    FUNCTION.remove(OIMessage(CMD_GET_TYPE_ALL, _id));
    ...
    FUNCTION.add(OIMessage(CMD_DIGITAL_READ, _id),   [this](OIMessage msg) -> uint32_t { 
        int getValue=0;
        getValue = digitalRead((Etor_t)msg.getConf());
        return getValue;
    });
    FUNCTION.add(OIMessage(CMD_ANALOG_READ, _id),   [this](OIMessage msg) -> uint32_t { 
        float getValue = analogRead((Eana_t)msg.getConf());
    ...
    FUNCTION.add(OIMessage(CMD_ATTACH_INTERRUPT, _id), [this](OIMessage msg) -> uint32_t { 
        ...
        static Etor_Args_t etor_args = {this, &etor};
        attachInterrupt(etor, _core_etor_isr_handler, &etor_args, (InterruptMode_t)msg.getData()); 
        return 0;
    });
    
    FUNCTION.add(OIMessage(CMD_DETACH_INTERRUPT, _id), [this](OIMessage msg) -> uint32_t { 
        detachInterrupt((Etor_t)msg.getConf(), (InterruptMode_t)msg.getData()); 
        return 0;
    });
}

void OICore::_core_etor_isr_handler(void* arg) 
{
    Etor_Args_t * etor_args = (Etor_Args_t *) arg;
    OICore * core =  etor_args->core;
    Etor_t * etor = etor_args->etor;
    InterruptMode_t mode = (core->digitalRead(*etor) == LOW ? FALLING : RISING );

    TWAI_S.sendMessage(OIMessage(CMD_ETOR_INTERRUPT, core->getId(), *etor, mode), BROADCAST_ID);
}