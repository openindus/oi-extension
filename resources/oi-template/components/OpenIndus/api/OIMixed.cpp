...
void OIMixed::init()
{
    ...
    gpio_set_level(OIMIXED_PIN_CMD_MOSFET, LOW);
...
{
    return Ads866x_AnalogRead(ana);
}

float OIMixed::analogRead(Eana_t ana, Units_t units)
{
    return Ads866x_AnalogReadUnits(ana, (Ads866x_Units_t)units);
}

void OIMixed::analogReadMode(Eana_t ana, AdcMode_t mode)
{
    Ads866x_setAdcMode(ana, (Ads866x_AdcMode_t) mode);
}
...
void IRAM_ATTR OIMixed::_mixed_isr_handler(void* arg)
{
...
void OIMixed::_mixed_isr_task(void* arg)
{
    ...
    for(;;) 
    {
        if(xQueueReceive(_mixed_evt_queue, &etor, portMAX_DELAY)) 
        {
                ...
                mode = (mixed->digitalRead(etor) == LOW ? FALLING : RISING );
                TWAI_S.sendMessage(OIMessage(CMD_ATTACH_INTERRUPT, mixed->getId(), etor, mode), BROADCAST_ID);
...
void OIMixed::attachFunctions(void)
{
    OIModule::attachFunctions();

    FUNCTION.add(OIMessage(CMD_DIGITAL_WRITE, _id), [this](OIMessage msg) -> uint32_t { digitalWrite((Stor_t)msg.getConf(), (uint8_t)msg.getData()); return 0;});
    FUNCTION.add(OIMessage(CMD_DIGITAL_READ, _id),   [this](OIMessage msg) -> uint32_t { 
        int getValue=0;
        getValue = digitalRead((Etor_t)msg.getConf(0));
        return getValue;
    });    
    FUNCTION.add(OIMessage(CMD_ANALOG_WRITE, _id),  [this](OIMessage msg) -> uint32_t { analogWrite((Stor_t)msg.getConf(), (uint8_t)msg.getData()); return 0;});
    FUNCTION.add(OIMessage(CMD_ATTACH_INTERRUPT, _id), [this](OIMessage msg) -> uint32_t { 
        attachInterrupt((Etor_t)msg.getConf(0), _mixed_isr_handler, (InterruptMode_t)msg.getData()); 
        return 0;
    });

    FUNCTION.add(OIMessage(CMD_DETACH_INTERRUPT, _id), [this](OIMessage msg) -> uint32_t { 
        detachInterrupt((Etor_t)msg.getConf(0), (InterruptMode_t)msg.getData()); 
        return 0;
    });
   
    FUNCTION.add(OIMessage(CMD_STOR_SENSOR_READ, _id),   [this](OIMessage msg) -> uint32_t { 
        float getValue = getStorCurrent((Stor_t)msg.getConf(0));
        uint32_t castedValue = reinterpret_cast<uint32_t &>(getValue);
        return castedValue;
    });
    FUNCTION.add(OIMessage(CMD_VOLTAGE_WRITE_DAC, _id), [this](OIMessage msg) -> uint32_t { 
        uint32_t data32 = msg.getData();
        float* data = reinterpret_cast<float*>(&data32);
        voltageWrite((Sana_t)msg.getConf(0), *data, (Units_t)msg.getConf(1));
        return 0;
    });
    FUNCTION.add(OIMessage(CMD_ANALOG_READ_ADC, _id),   [this](OIMessage msg) -> uint32_t { 
        float getValue = analogRead((Eana_t)msg.getConf(0), (Units_t)msg.getConf(1));
        uint32_t castedValue = reinterpret_cast<uint32_t &>(getValue);
        return castedValue;
    });
    FUNCTION.add(OIMessage(CMD_GET_ANALOG_ADC_REFERENCE, _id),   [this](OIMessage msg) -> uint32_t { 
        float getValue = getAnalogReference((Units_t)msg.getConf(1));
        uint32_t castedValue = reinterpret_cast<uint32_t &>(getValue);
        return castedValue;
    });
    FUNCTION.add(OIMessage(CMD_SET_ANALOG_ADC_MODE, _id), [this](OIMessage msg) -> uint32_t { 
        analogReadMode((Eana_t)msg.getConf(0), (AdcMode_t)msg.getData(0));
        return 0;
    });
    FUNCTION.add(OIMessage(CMD_SET_ANALOG_ADC_RESOLUTION, _id), [this](OIMessage msg) -> uint32_t { 
        analogReadResolution((AdcRes_t)msg.getData(0));
        return 0;
    });
    FUNCTION.add(OIMessage(CMD_SET_ANALOG_DAC_MODE, _id), [this](OIMessage msg) -> uint32_t { 
        setAnalogMode((Sana_t)msg.getData(0), (DACAnalogMode_t)msg.getData(0));
        return 0;
    });
    FUNCTION.add(OIMessage(CMD_GET_ANALOG_DAC_MODE, _id), [this](OIMessage msg) -> uint32_t { 
        DACAnalogMode_t mode = getAnalogMode((Sana_t)msg.getData(0));
        return reinterpret_cast<uint32_t &>(mode);
    });
    FUNCTION.add(OIMessage(CMD_SET_CURRENT_DAC_MODE, _id), [this](OIMessage msg) -> uint32_t { 
        setCurrentMode((Sana_t)msg.getData(0), (DACCurrentMode_t)msg.getData(0));
        return 0;
    });
    FUNCTION.add(OIMessage(CMD_GET_CURRENT_DAC_MODE, _id), [this](OIMessage msg) -> uint32_t { 
        DACCurrentMode_t mode = getCurrentMode((Sana_t)msg.getData(0));
        return reinterpret_cast<uint32_t &>(mode);
    });
    FUNCTION.add(OIMessage(CMD_CURRENT_WRITE_DAC, _id), [this](OIMessage msg) -> uint32_t { 
        uint32_t data32 = msg.getData();
        float* data = reinterpret_cast<float*>(&data32);
        currentWrite((Sana_t)msg.getConf(0), *data, (Units_t)msg.getConf(1));
        return 0;
    });
}
