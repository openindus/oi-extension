...
void OIDiscrete::init()
{
    ...
    gpio_set_level(OIDISCRETE_PIN_CMD_MOSFET, LOW);
...
void IRAM_ATTR OIDiscrete::_discrete_isr_handler(void* arg)
{
...
void OIDiscrete::_discrete_isr_task(void* arg)
{
    ...
    for(;;) 
    {
        if(xQueueReceive(_discrete_evt_queue, &etor, portMAX_DELAY)) 
        {
                ...
                mode = (discrete->digitalRead(etor) == LOW ? FALLING : RISING );
                TWAI_S.sendMessage(OIMessage(CMD_ETOR_INTERRUPT, discrete->getId(), etor, mode), BROADCAST_ID);
...
void OIDiscrete::attachFunctions(void)
{
    ...
    FUNCTION.add(OIMessage(CMD_ANALOG_READ, _id),   [this](OIMessage msg) -> uint32_t { 
     ...
     FUNCTION.add(OIMessage(CMD_STOR_SENSOR_READ, _id),   [this](OIMessage msg) -> uint32_t { 
        float getValue = getStorCurrent((Stor_t)msg.getConf());
        uint32_t valueReturn = 0;
        memcpy(&valueReturn, &getValue, sizeof(float));
        return valueReturn;
    });
    ...
    FUNCTION.add(OIMessage(CMD_ATTACH_INTERRUPT, _id), [this](OIMessage msg) -> uint32_t { 
        attachInterrupt((Etor_t)msg.getConf(), _discrete_isr_handler, (InterruptMode_t)msg.getData()); 
        return 0;
    });
