...
/* ETOR */
#define OIMIXED_PIN_ETOR1           (GPIO_NUM_19)
#define OIMIXED_PIN_ETOR2           (GPIO_NUM_20)
#define OIMIXED_PIN_ETOR3           (GPIO_NUM_26)
#define OIMIXED_PIN_ETOR4           (GPIO_NUM_46)

/* STOR */
#define OIMIXED_PIN_STOR1           (GPIO_NUM_38)
#define OIMIXED_PIN_STOR2           (GPIO_NUM_42)
#define OIMIXED_PIN_STOR3           (GPIO_NUM_45)
...
public:
     ...
     */
    void attachInterrupt(Etor_t etor, voidFuncPtrArg handler, InterruptMode_t mode = RISING);
...
private:
    ...
    static void IRAM_ATTR _mixed_isr_handler(void* arg);
    static void _mixed_isr_task(void* arg);    
};

#else

class OIMixed : public OISubModule
{
public:

    OIMixed(uint8_t id) : OISubModule(id) {}
    ...
    inline void digitalWrite(Stor_t stor, uint8_t level) {
        setMessage(OIMessage(CMD_DIGITAL_WRITE, _senderId, 0, (uint8_t)stor, (uint32_t)(level & 1)));
    }
    ...
    inline void analogWrite(Stor_t stor, uint8_t duty) {
        setMessage(OIMessage(CMD_ANALOG_WRITE, _senderId, 0, (uint8_t) stor, (uint32_t) duty));
    }
    ...
    inline int digitalRead(Etor_t etor) const {
        return (int)getMessage(OIMessage(CMD_DIGITAL_READ, _senderId, 0, (uint8_t) etor));
    }
    ...
    inline float getStorCurrent(Stor_t stor) const {
        uint32_t value = getMessage(OIMessage(CMD_STOR_SENSOR_READ, _senderId, 0, (uint8_t) stor));
    ...
    inline void attachInterrupt(Etor_t etor, void (*callback)(void), InterruptMode_t mode) {
        setMessage(OIMessage(CMD_ATTACH_INTERRUPT, _senderId, 0, (uint8_t) etor, (uint8_t) mode));
        FUNCTION.add(OIMessage(CMD_ETOR_INTERRUPT, _senderId), [callback](OIMessage msg) -> uint32_t {
            callback();
            return 0;
        });
    }
    ...
    inline void detachInterrupt(Etor_t etor, InterruptMode_t mode) {
        setMessage(OIMessage(CMD_DETACH_INTERRUPT, _senderId, 0, (uint8_t) etor, (uint8_t) mode));
    }
    ...
    {
        return getMessage(OIMessage(CMD_ANALOG_READ_ADC, _senderId, (uint8_t)UNITS_RAW, (uint8_t) ana));
    }
    ...
    {
        uint32_t value = getMessage(OIMessage(CMD_ANALOG_READ_ADC, _senderId, (uint8_t)units, (uint8_t) ana));
        return reinterpret_cast<float &>(value);
    }
    ...
    {
        setMessage(OIMessage(CMD_SET_ANALOG_ADC_MODE, _senderId, 0, (uint8_t) ana, (uint8_t) mode));
    }
    ...
    {
        setMessage(OIMessage(CMD_SET_ANALOG_ADC_RESOLUTION, _senderId, 0, 0, (uint8_t) res));
    }
    ...
    {
        uint32_t value = getMessage(OIMessage(CMD_GET_ANALOG_ADC_REFERENCE, _senderId, (uint8_t)units, 0));
        return reinterpret_cast<float &>(value);        
    }
    ...
    {
        setMessage(OIMessage(CMD_SET_ANALOG_DAC_MODE, _senderId, 0, (uint8_t) sana, (uint8_t) mode));
    }
    ...
    {
        uint32_t value = getMessage(OIMessage(CMD_GET_ANALOG_DAC_MODE, _senderId, 0, (uint8_t)sana));
    ...
    {
        setMessage(OIMessage(CMD_VOLTAGE_WRITE_DAC, _senderId, (uint8_t)UNITS_RAW, (uint8_t) sana, value));
    }
    ...
    inline void voltageWrite(Sana_t sana, float value, Units_t units)
    {
        ...
        setMessage(OIMessage(CMD_VOLTAGE_WRITE_DAC, _senderId, (uint8_t)units, (uint8_t) sana, val));
    }
    ...
    {
        setMessage(OIMessage(CMD_SET_CURRENT_DAC_MODE, _senderId, 0, (uint8_t) sana, (uint8_t) mode));
    }
    ...
    {
        uint32_t value = getMessage(OIMessage(CMD_GET_CURRENT_DAC_MODE, _senderId, 0, (uint8_t)sana));
    ...
    {
        setMessage(OIMessage(CMD_CURRENT_WRITE_DAC, _senderId, (uint8_t)UNITS_RAW, (uint8_t) sana, value));
    }
    ...
    {
        uint32_t val = reinterpret_cast<uint32_t &>(value);
        setMessage(OIMessage(CMD_CURRENT_WRITE_DAC, _senderId, (uint8_t)units, (uint8_t) sana, val));
    }
